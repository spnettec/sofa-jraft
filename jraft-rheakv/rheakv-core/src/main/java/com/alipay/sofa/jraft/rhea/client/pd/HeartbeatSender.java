...
public class HeartbeatSender implements Lifecycle<HeartbeatOptions> {
    ...
    public synchronized boolean init(final HeartbeatOptions opts) {
        ...
        this.heartbeatRpcCallbackExecutor = ThreadPoolUtil.newBuilder().poolName(name).enableMetric(true)
            .coreThreads(4).maximumThreads(4).keepAliveSeconds(120L).workQueue(new ArrayBlockingQueue<>(1024))
            .threadFactory(new NamedThreadFactory(name, true)).rejectedHandler(new DiscardOldPolicyWithReport(name))
            .build();
